...
export const loadAchievements = async (
...
): Promise<Array<{ id: string; paths: Array<SkPath> | null }>> => {
  ...
  for (let i = 0; i < achievements.length; i++) {
    ...
    const result: { id: string; paths: Array<SkPath> | null } = {
      id: a.id,
      paths: null,
    };
    if (file) {
...
export const prepareAchievements = async (
  ...
  if (!stampsColors || !bgImage || !themeName || !screenWidth || !screenHeight) return { image: null, data: [] };
  ...
  function addStamp(
    ...
    radians: number,
    polys: Array<Polygon>,
  ): { left: number; top: number; radians: number } {
  ...
  try {
    let width = Math.round(screenWidth);
    let height = Math.round(screenHeight) - 300;

    const list = await getAchievements();
    const achievements: Achievement[] = list.map((a) => {
      return {
        arrivalAirport: a.arrivalAirport,
        date: new Date(a.flightDate).toLocaleString(undefined, {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        }),
    ...
    for (let i = 0; i < svgs.length; i++) {
      ...
      const { left, top, radians } = addStamp(
        width,
        size,
        r,
        stamps.map((s) => s.hull),
...
export const createCachedImage = async (
  width: number,
  height: number,
  bgImage: SkImage,
  stamps: Stamp[],
  ...
  if (!width || !height || !bgImage) return null;
