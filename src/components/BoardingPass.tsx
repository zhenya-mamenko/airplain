...
import { azteccode, pdf417, qrcode, datamatrix, RenderOptions } from '@bwip-js/react-native';
...
const BarCode = ({ format, options, ...rest }: BarCodeProps) => {
  const [img, setImg] = React.useState<{ uri: string; height: number; width: number } | null>(null);
...
const BoardingPass = (props: {pkpass: PKPassData}) => {
  ...
  const Field = ({ label, value, ...rest}: FieldProps) => {
    return (
      <View
        { ...rest }
        className={ rest.className ? rest.className + ' flex-1 flex-column' : 'flex-1 flex-column' }
      >
        <Text
          { ...rest.labelProps }
          className={(rest.labelProps as any).className ? (rest.labelProps as any).className + ' size-sm' : 'size-sm'}
          style={{ color: colors.labelColor, ...(rest.labelProps as any).style }}
        >
          { label }
        </Text>
        <Text
          ...
          style={{ color: colors.foregroundColor, ...(rest.valueProps as any).style }}
        >
          { value }
        </Text>
      </View>
    ...
    if (asset && asset.image) {
      ...
      image = (
        <Image
          { ...rest }
          source={data}
          style={{ width: data.width / scale, height: data.height / scale, ...rest.style }}
        />
      );
    }
    return image;
  ...
  return (
    ...
    >
      <View
        className='flex-row alignitems-start justifycontent-between'
      >
        {
          images.logo ?
        ...
        { boardingPass.headerFields ?
          <Fields fields={boardingPass.headerFields} className='alignitems-end' />
          : null
        }
      </View>
      { boardingPass.primaryFields && boardingPass.primaryFields.length == 2 ?
        <View
          className='flex-row alignitems-end justifycontent-between mt-lg mb-md'
        >
          <Field
            ...
            valueProps={{ style: { color: colors.foregroundColor, fontSize: 48 } }}
          />
          <View
          ...
          <Field
            ...
            valueProps={{ className: 'align-right', style: { color: colors.foregroundColor, fontSize: 48 } }}
          />
        </View>
        : null
      }
      ...
      />
      {
        barcode && barcode.message ?
          <View
            className='flex-1 flex-column alignitems-center justifycontent-start'
          >
            <View
