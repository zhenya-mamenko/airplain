...
export default function YearSelector(props: { current?: string, years: string[], onYearChange: (year: string) => void }) {
  const yearData: YearData[] = props.years.map(
    year => ({ key: year, value: year === 'all' ? t('flights.all').toLocaleUpperCase() : year.toString() })
  );
  ...
  const renderItem = (item: YearData, isSelected: boolean) => {
    return (
        ...
        >
          <Text
            className={`size-md weight-bold color-primaryContainer`}
          >
