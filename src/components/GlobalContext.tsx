...
interface GlobalProps {
  ...
  setAchievements: (newAchievements: { image: SkImage | null, data: ContextData[] }) => void;
};
...
export const GlobalContextProvider = forwardRef(({ children }: { children: any }, ref) => {
  const [achievements, setAchievements] = useState<{ image: SkImage | null | undefined, data: ContextData[] }>({ image: undefined, data: [] });
  const [flightsFilter, setFlightsFilter] = useState<FlightsFilter>({ airports: [], dateFrom: undefined, dateTo: undefined });
  ...
  for (const key in stampsColors) {
    ['gold', 'blue', 'blue_extra', 'purple', 'red', 'red_extra', 'green', 'green_extra', 'black'].forEach((x: string) => {
      stampsColors[key as keyof typeof stampsColors][x] = useThemeColor(`stamps.${x}`, key as any);
    });
  }

  return (
    <GlobalContext.Provider value={{ achievements, flightsFilter, stampsColors, setFlightsFilter, setAchievements }}>
      { children }
