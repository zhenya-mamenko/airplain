...
export default function AddFlight(props: { today?: Date }) {
  ...
  const processManuallyAdd = async () => {
    ...
    const flight: Flight = {
      ...
      distance: Math.round(
        haversine(
          departureAirport?.airport_latitude ?? 0,
          departureAirport?.airport_longitude ?? 0,
          arrivalAirport?.airport_latitude ?? 0,
          arrivalAirport?.airport_longitude ?? 0,
        ),
      ),
  ...
  const findFlight = async () => {
    setProcessing(true);
    const flightId = await isFlightExists(
          ...
          >
                ...
                <View className="flex-row">
                  ...
                  <View className="flex-3">
                    <Select
                      ...
                      data={getAirlinesData().map((x: any) => {
                        x['full'] = `${x.airlineCode} ${x['airlineName']}`;
                        return x;
                      })}
                      ...
                      onChange={(item: any) => {
                        dispatch({
                          type: 'search',
                          value: { airline: item.airlineCode },
                        });
                        dispatch({
                          type: 'bcbp',
                          value: { data: undefined, pkpass: undefined },
                        });
            ...
            {state.visibility.addManuallyForm && (
              <View className="bg-surfaceVariant alignitems-start justifycontent-start p-md flex-column flex-1">
                    ...
                    <TextInput
                      ...
                      onBlur={() => {
                        ...
                        if (airline) {
                          dispatch({
                            type: 'add',
                            value: { airlineName: airline.airlineName },
                          });
                        }
                      }}
                      onChangeText={(text: string) => {
                        dispatch({
                          type: 'add',
                          value: { airline: text.toLocaleUpperCase() },
                        });
                      }}
                ...
                <View className="flex-row mb-md">
                  <View className="flex-column flex-2 pr-xxl">
                    <Text
                      className="size-md color-primaryContainer mb-sm mt-md"
                      style={{ fontVariant: ['small-caps'] }}
                    >
