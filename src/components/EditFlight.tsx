    ...
    <View className="flex-row my-sm">
      ...
      >
        ...
        <DatetimeInput
          ...
          timezone={
            type === 'End'
              ? state.arrivalAirportTimezone
              : state.departureAirportTimezone
          }
          value={value}
...
const Card: React.FC<{ dataCards: any; item: string; dragEnabled: boolean }> =
  React.memo(({ dataCards, item, dragEnabled }) => {
...
const EditFlight = React.memo((props: { data: Flight }) => {
      ...
      <View className="flex-column">
        <View className="flex-row my-sm justifycontent-between alignitems-start">
          <View className="flex-column" style={{ maxWidth: '80%' }}>
            ...
            <Text className="size-sm color-surface mt-xs">
  ...
  const entries = FlightStatusValues.map((x) => [
    x,
    t(`flights.statuses.${x}`),
  ]);
            ...
            value={
              <View className="flex-row justifycontent-start alignitems-center mt-xs">
                <Image
                  ...
                  style={{ width: 20, height: 20, backgroundColor: '#FFFFFF' }}
                />
                <Text className="size-smm weight-bold ml-sm color-surface">
  ...
  const renderItem = ({ item }: ListRenderItemInfo<DCType>) => {
    return item == 'none' ? (
      <View
        style={{ height: 75, width: '100%', backgroundColor: 'transparent' }}
      />
    ) : (
