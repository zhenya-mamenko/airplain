...
import { DataCard, DataCardContext, Input, Select, Value } from '@/components/DataCard';
...
const DateTimeRow = ({ type, state }: { type: 'Start' | 'End'; state: any }) => {
...
const Card: React.FC<{ dataCards: any; item: string; dragEnabled: boolean }> = React.memo(
  ({ dataCards, item, dragEnabled }) => {
    const drag = useReorderableDrag();
    const props = dragEnabled ? { onLongPress: drag } : {};
    return <Pressable {...props}>{dataCards[item]}</Pressable>;
  },
);
...
const EditFlight = React.memo((props: { data: Flight }) => {
  ...
  useEffect(() => {
    navigation?.setOptions({
      headerRight: () => (
        <>
          <Pressable onPress={() => getFlightDataFromApi()}>
            <Icon name="cloud-download-alt" size={16} color={colorPrimary} style={{ marginTop: 4, marginRight: 16 }} />
          </Pressable>
          <Pressable onPress={() => setDragEnabled(!dragEnabled)}>
            <Icon
              name={!dragEnabled ? 'bars' : 'copy'}
              size={16}
              color={colorPrimary}
  ...
  const departureDataCard = () => (
    <DataCard
      caption={
        <View className="flex-row alignitems-end">
          ...
          <Text className="size-smm weight-bold mt-xs color-secondaryContainer ml-smm">
    ...
    >
      <View className="flex-column">
        <View className="flex-row my-sm justifycontent-between alignitems-start">
          <View className="flex-column" style={{ maxWidth: '80%' }}>
            <Text className="size-mdl weight-bold color-surface" ellipsizeMode="tail" numberOfLines={1}>
              {`${departureAirportData?.municipality_name} (${state.departureAirport})`}
            </Text>
            <Text className="size-sm color-surface mt-xs">{departureAirportData?.airport_name}</Text>
          </View>
          <Text className="alignself-start ml-mdl" style={{ lineHeight: 44, fontSize: 40 }}>
            {departureFlag}
          </Text>
        </View>
        <View className="flex-row mb-sm">
          <Value
            ...
            value={departureDate.toLocaleDateString(locale, {
              ...dateOptions,
              timeZone: state.departureAirportTimezone ?? 'UTC',
            })}
            width="66%"
          />
          <Value
            caption={t('add.time')}
            selectable={!dragEnabled}
            value={departureDate.toLocaleTimeString(locale, {
              ...timeOptions,
              timeZone: state.departureAirportTimezone ?? 'UTC',
            })}
            width="34%"
        ...
        <View className="flex-row mb-sm">
          <Value
            ...
            width="33%"
          />
          <Value caption={t('flights.gate')} selectable={!dragEnabled} value={none(state.departureGate)} width="33%" />
          <Value
      ...
      <View className="flex-column">
        ...
        <View className="flex-row mb-sm">
          <Input
            ...
            width="33%"
          />
          <Input caption={t('flights.gate')} field="departureGate" value={state['departureGate']} width="33%" />
          <Input
            caption={t('flights.desk')}
  ...
  const arrivalDataCard = () => (
    ...
    >
      <View className="flex-column">
        <View className="flex-row my-sm justifycontent-between alignitems-start">
          <View className="flex-column" style={{ maxWidth: '80%' }}>
            <Text className="size-mdl weight-bold color-surface">
              {`${arrivalAirportData?.municipality_name} (${state.arrivalAirport})`}
            </Text>
            <Text className="size-sm color-surface mt-xs">{arrivalAirportData?.airport_name}</Text>
          </View>
          <Text className="lignself-start ml-mdl" style={{ lineHeight: 44, fontSize: 40 }}>
            {arrivalFlag}
          </Text>
        </View>
        ...
        <View className="flex-row mb-sm">
          <Value
            caption={t('add.date')}
            selectable={!dragEnabled}
            value={arrivalDate.toLocaleDateString(locale, {
              ...dateOptions,
              timeZone: state.arrivalAirportTimezone ?? 'UTC',
            })}
            width="66%"
          />
          <Value
            caption={t('add.time')}
            selectable={!dragEnabled}
            value={arrivalDate.toLocaleTimeString(locale, {
              ...timeOptions,
              timeZone: state.arrivalAirportTimezone ?? 'UTC',
            })}
            width="34%"
      ...
      <View className="flex-column">
        <DateTimeRow state={state} type="End" />

        <View className="flex-row mb-sm">
          <Input caption={t('flights.terminal')} field="arrivalTerminal" value={state['arrivalTerminal']} width="66%" />
          <Input caption={t('flights.baggage_belt')} field="baggageBelt" value={state['baggageBelt']} width="34%" />
        </View>
      </View>
    </DataCard>
  );
  ...
  const boardingpassDataCard = () => (
    ...
    >
      <View className="flex-column">
        <View className="flex-row my-sm">
          <Value caption={t('flights.pnr')} selectable={!dragEnabled} value={none(state.pnr)} width="66%" />
          <Value caption={t('flights.seat')} selectable={!dragEnabled} value={none(state.seatNumber)} width="34%" />
        </View>
      </View>
      <View className="flex-column">
        <View className="flex-row my-sm">
          <Input caption={t('flights.pnr')} field="pnr" value={state['pnr']} width="66%" />
          <Input caption={t('flights.seat')} field="seatNumber" value={state['seatNumber']} width="34%" />
        </View>
        <View className="flex-column my-sm">
          <LoadBCBPOptions dispatch={bcbpDispatch} showToast={false} />
        </View>
      </View>
    </DataCard>
  );
    ...
    <>
      <View className="flex-row my-sm">
        <Value
          ...
          width="66%"
        />
        <Value caption={t('add.flight_number')} value={`${state.airline} ${state.flightNumber}`} width="34%" />
      </View>
      {state.extra.carrier && (
        <View className="flex-row mb-sm">
          <Value
      ...
      <View className="flex-column">
        ...
        <View className="flex-row mb-sm">
          <View style={{ width: '100%' }} className="pr-md">
            <Select
              caption={t('flights.status')}
              data={Object.entries(flightStatuses).map((x) => ({
                id: x[0],
                value: x[1],
              }))}
              field="status"
  ...
  const aircraftDataCard = () => (
    <DataCard
      ...
      onSave={dataCardOnSave}
    >
      <View className="flex-column mt-sm">
        <View className="flex-row mb-sm">
          <Value caption={t('flights.aircraft_type')} selectable={!dragEnabled} value={none(state.aircraftType)} />
        </View>
        <View className="flex-row mb-sm">
          <Value
      ...
      <View className="flex-column">
        <View className="flex-row my-sm">
          <Input caption={t('flights.aircraft_type')} field="aircraftType" value={state['aircraftType']} width="100%" />
        </View>
        <View className="flex-row mb-sm">
          <Input
            caption={t('flights.aircraft_registration_number')}
  ...
  const notesDataCard = () => (
    ...
    >
      <View className="flex-row my-sm">
        <Value caption="" lines={4} selectable={!dragEnabled} value={none(state['notes'])} />
      </View>
      <View className="flex-row my-sm">
        <Input caption="" field="notes" lines={4} value={state['notes']} />
      </View>
    </DataCard>
  );
  ...
  const renderItem = ({ item }: ListRenderItemInfo<DCType>) => {
    return item == 'none' ? (
      <View style={{ height: 75, width: '100%', backgroundColor: 'transparent' }} />
    ) : (
      <Card dataCards={dataCards} item={item} dragEnabled={dragEnabled} />
    );
  };
  const handleReorder = ({ from, to }: ReorderableListReorderEvent) => {
    setData((value) => {
      const newData = to === FLIGHT_CARDS.split('|').length - 1 ? [...value] : reorderItems(value, from, to);
