...
export default function LandedFlightCard(props: { data: LandedFlightCardData }) {
  ...
  const { airline, flightNumber, startDatetime, actualStartDatetime, departureAirport, arrivalAirport,
    actualEndDatetime, endDatetime, departureTerminal, arrivalTerminal, isDifferentTimezone, baggageBelt,
    departureAirportTimezone, arrivalAirportTimezone, flightId, isArchived } = props.data;
  ...
  }, [lat, lon, network.isInternetReachable, settings.TEMPERATURE_UNITS, settings.TEMPERATURE_TYPE, settings.WEATHER_API_KEY]);
  useEffect(() => {
    loadWeatherCallback();
  }, [loadWeatherCallback]);
  ...
  const flag = arrivalAirportData?.country_code !== departureAirportData?.country_code ?
  ...
  return (
    <GestureDetector gesture={gestures}>
      <View collapsable={false}>
        <Animated.View
          style={[{ marginBottom: 16, borderRadius: 12, borderColor: colorSurfaceVariant, borderWidth: 2 }, animatedStyle]}
        >
          <View
          ...
          >
            ...
            <View
              className='flex-row px-md justifycontent-between alignitems-start pb-md'
            >
              ...
              >
                { baggageBelt &&
                  <View
                    className='flex-row justifycontent-center alignitems-center radius-md bg-primaryContainer py-xs pr-md'
                  >
                    <SvgBaggageBelt
                      color={ colorPrimaryContainer }
                      style={{ width: 40, height: 40, marginLeft: 12, marginTop: -4, marginBottom: -4 }}
                    />
                    <Text
            ...
            >
              ...
              <View
                className='flex-row justifycontent-end alignitems-center alignself-end'
              >
                { airportWeather &&
                  <Text
                    className='size-lg color-primaryContainer align-right'
                  >
