...
const PlaceBlock = React.memo((props: {
  actual: number,
  airport: string,
  align: 'start' | 'end',
  city: string,
  ...
  const actualTime = (
    <Text
      className={
        `size-md color-surface alignself-end weight-bold m${props.align === 'start' ? 'l' : 'r'}-sm ${!!timeClass ? `color-${timeClass}` : ''}`
      }
    >
      { props.date }
    </Text>
  ...
  return (
    <View className='flex-column' style={{ width: '40%'}}>
      ...
      <View
        className={ `flex-row justifycontent-${props.align}` }
      >
...
const RouteSymbol = React.memo((props: { status: FlightStatus }) => {
  switch (props.status) {
    ...
    case 'arrived':
      return (
        <View
          className='flex-row justifycontent-around alignitems-center'
        >
          <SvgLine
    ...
    case 'en_route':
        return (
        <View
          className='flex-row justifycontent-center alignitems-center'
        >
          <SvgArrowLong
            color={ useThemeColor('textColors.surface') }
...
const FlightCard = React.memo((props: { data: FlightCardData, noGestures?: boolean }) => {
  const { departureAirport, arrivalAirport, actualEndDatetime, actualStartDatetime, endDatetime, startDatetime,
    status, distance, departureAirportTimezone, arrivalAirportTimezone, airline, flightNumber, flightId, isArchived,
    isOnlineCheckInOpen, onlineCheckInLink, seatNumber, state, stateTime } = props.data;
  ...
  const dateLabel = makeDateLabel(departureDate, departureAirportTimezone ?? 'UTC', arrivalDate, arrivalAirportTimezone ?? 'UTC', locale);
  ...
  const component = (
    <View collapsable={false}>
      ...
      >
        <View
          className='radiustr-md radiustl-md px-md py-sm bb-1 bordercolor-outlineVariant bg-secondaryContainer flex-row justifycontent-between alignitems-center'
        >
          <View className='flex-row justifycontent-start alignitems-center'>
            <Image
              ...
              style={{ width: 20, height: 20, backgroundColor: '#FFFFFF', marginTop: 2 }}
            />
            <Text
              className='size-smm weight-bold mt-xs ml-sm color-gray'
            >
              { `${airline} ${flightNumber}` }
            </Text>
          </View>
          <Text
            className='size-sm weight-bold mt-xs color-gray'
          >
            { `${dateLabel.toLocaleUpperCase()}` }
          </Text>
        </View>
        <View
          className='px-md pb-md pt-md radiusbr-md radiusbl-md bg-background flex-row justifycontent-between'
        >
          <PlaceBlock
  ...
  return !!props.noGestures ? component : (
...
}, (prevProps: {data: FlightCardData, noGestures?: boolean }, nextProps: {data: FlightCardData, noGestures?: boolean }) => {
  return prevProps.noGestures === nextProps.noGestures &&
