...
const Card: React.FC<CardProps> = (props) => {
    ...
    function createContent() {
      ...
      v.forEach((v, i) => {
        c.push(
          <Animated.Text
            key={`value${i}`}
            style={{ fontWeight: 'bold', color: valueColor, fontSize: sizePx, fontVariant: ['tabular-nums'] }}
          >
            {v}
          </Animated.Text>
        );
  ...
  return (
    <View className={`flex-column alignitems-start justifycontent-start flex-${flex} py-xs
      ...
      <Animated.View
        style={[animatedStyle, { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'flex-start' }]}
      >
        {content}
      </Animated.View>
    </View>
...
export default function Stats() {
  ...
  const countryCodes = stats?.[year]?.countryCodes?.split(',').map((code) => flags.find(x => x.country_code === code)?.flag).join('');
  ...
  const flightsStyle: TextStyle = {
    width: 30, fontSize: 12, color: valueColor, fontWeight: 'bold', textAlign: 'right',
              ...
              <View className='flex-column alignitems-start justifycontent-start bg-surface m-sm radius-md b-1 bordercolor-outline pb-sm elevated'>
                <View className='flex-row radiustr-md radiustl-md p-md bb-1 bordercolor-outlineVariant bg-secondaryContainer'>
                  <Text className='size-lg color-surface ml-xs flex-1'>
                    {t('stats.caption', {period: year === 'all' ? t('stats.all_time') : year, y: year !== 'all' ? t('measurements.year') : ''})}
                  </Text>
                </View>
                <View className='flex-row px-md pt-md pb-sm'>
                  ...
                  />
                  <View
                    className='flex-column alignitems-end justifycontent-end mb-smm'
                  >
                ...
                <View className='flex-row px-md pb-sm'>
                  ...
                  />
                  <View
                    className='flex-2 py-sm justifycontent-end alignitems-start'
                  >
                    <Animated.Text
                      ellipsizeMode='clip'
                      numberOfLines={2}
